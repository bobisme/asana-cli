...
impl AsanaTaskRepository {
    ...
    fn build_task_query_params(&self, filter: &TaskFilter) -> Vec<(String, String)> {
        ...
        params.push((
            ...
            "gid,name,notes,completed,due_on,due_at,assignee.gid,assignee.name,assignee.email,projects.gid,projects.name,tags.gid,tags.name,created_at,modified_at,workspace.gid,workspace.name".to_string(),
        ));
...
impl TaskRepository for AsanaTaskRepository {
    async fn get_task(&self, id: &TaskId) -> RepositoryResult<Task> {
        let path = format!(
            "/tasks/{}?opt_fields=gid,name,notes,completed,due_on,due_at,assignee.gid,assignee.name,assignee.email,projects.gid,projects.name,tags.gid,tags.name,created_at,modified_at,workspace.gid,workspace.name",
            id.0
        );
